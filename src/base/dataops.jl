...
function dselect(
    dInfo::LoadedDataInfo,
    currentColnames::Vector{String},
    selectColnames::Vector{String},
    tgt = dInfo.val,
)::LoadedDataInfo
...
function dapply_cols(dInfo::LoadedDataInfo, fn, columns::Vector{Int})
    transform_columns = x -> for (idx, c) in enumerate(columns)
        x[:, c] = fn(x[:, c], idx)
    end
...
function dapply_rows(dInfo::LoadedDataInfo, fn)
    transform_rows = x -> for i = 1:size(x, 1)
        x[i, :] = fn(x[i, :])
    end
...
function mapbuckets(
    ...
    extent = [i == bucketdim ? (0 .== buckets) : (1:size(a, i)) for i = 1:ndims]
