...
@testset "File splitting" begin
    ...
    for splitSize = 1:8
        di = load_fcs_distributed(:test, md[:, :file_name], W[1:splitSize])
        ...
        sizes = read_fcs_sizes(md[:, :file_name])
        dis = fcs_filevector_distribute(:testF, md[:, :file_name], W[1:splitSize])
